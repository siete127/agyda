...
import { FileSpreadsheet, CheckCircle2, XCircle, Clock, Sparkles } from 'lucide-react'
...
export function PortalClienteCotizacionesPage() {
  ...
  // Solo se muestran los tabs de estatus que realmente tienen alguna
  // cotización — "Todas" siempre aparece como referencia del total.
  const filtrosVisibles = FILTROS.filter((f) => f === 'todas' || cotizaciones.some((c) => c.estatus === f))
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center gap-3 py-10 text-center">
            ...
            {cotizaciones.length === 0 && (
              <button
                type="button"
                onClick={() => navigate('/portal-cliente/productos?nueva=1')}
                className="mt-1 flex items-center gap-2 rounded-xl bg-gradient-to-br from-[#19b6bc] to-[#00537f] px-4 py-2.5 text-sm font-bold text-white shadow-card transition-opacity hover:opacity-90"
              >
                <Sparkles className="h-4 w-4" /> Solicitar cotización
              </button>
            )}
          </div>
        ) : (
