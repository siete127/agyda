...
import { CalendarCheck, RefreshCw, Search, Phone } from 'lucide-react'
...
type Rol = 'paterno' | 'materno' | 'nombres' | 'telefono' | 'contacto' | 'asistencia' | 'horario' | 'puesto' | 'estatus' | 'asesor'
...
export default function RegistrosFormularioPage() {
  ...
  const conteo = {
    hoy: filas.filter((f) => f.asistencia === hoy).length,
    manana: filas.filter((f) => f.asistencia === manana).length,
    semana: filas.filter((f) => f.asistencia && f.asistencia >= hoy && f.asistencia <= en7).length,
  }
      ...
      <div className="overflow-hidden rounded-2xl border border-gray-200/60 bg-card shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-100 px-5 py-3">
          ...
          <span className="text-[0.72rem] text-gray-400">
            {visibles.length} de {data?.total ?? 0}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-100">
                {visibles.map((f) => {
                  ...
                  return (
                    <tr key={f.r.interaccionId} className={clsx('hover:bg-gray-50/60', esHoy && 'bg-emerald-50/50', pasada && 'text-gray-400')}>
                      {hayAsistencia && (
                      ...
                      <td className="px-4 py-2.5 font-semibold text-gray-800">{f.nombre}</td>
                      <td className="whitespace-nowrap px-4 py-2.5">
                        {f.telefono ? (
