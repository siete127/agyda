...
import { Contact, Check, Loader2, Info } from 'lucide-react'
import { clsx } from 'clsx'
import toast from 'react-hot-toast'
import { personalizacionService, type ProspeccionConfig } from '@/services/personalizacion.service'
...
const DEFAULTS: ProspeccionConfig = { ventanaAnalisisDias: 30 }

export function ProspeccionConfigTab() {
  ...
  const [form, setForm] = useState<ProspeccionConfig | null>(null)
  const [seededFrom, setSeededFrom] = useState<ProspeccionConfig | null>(null)
  const actual = data?.prospeccion ?? DEFAULTS
  ...
  const guardar = useMutation({
    mutationFn: async () => {
      if (!form) return
      await personalizacionService.updateProspeccion(form)
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['personalizacion'] })
      toast.success('Configuración de prospección actualizada')
    },
    ...
    <div className="space-y-5">
      <div className="rounded-2xl border border-gray-100 bg-card p-5 shadow-card">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h2 className="text-[1.35rem] font-bold text-gray-900">Prospección</h2>
            <p className="text-[0.82rem] text-gray-400">
              Ventana de tiempo usada para las estadísticas del dashboard de prospección.
            </p>
          </div>
        </div>
      </div>

      <CardSeccion icon={Contact} titulo="Ventana de análisis" subtitulo="Días hacia atrás usados en 'gestiones recientes' y 'tipos de gestión más comunes'.">
        <div className="flex items-center gap-2">
          <input type="number" min={1} max={365} step={1} className={clsx(numCls, 'max-w-[140px]')}
            value={form.ventanaAnalisisDias}
            onChange={(e) => setForm((f) => (f ? { ...f, ventanaAnalisisDias: Number(e.target.value) } : f))} />
          <span className="text-sm text-gray-400">días</span>
        </div>
        <p className="mt-2 text-[0.68rem] text-gray-400">Normalmente 30 días.</p>
      </CardSeccion>
      ...
      <div className="flex items-start gap-2 rounded-xl bg-violet-50/60 px-3 py-2.5">
        ...
        <p className="text-[0.72rem] text-gray-500">
          Esta configuración es propia de esta empresa y aplica en el dashboard de Prospección (CRM → Prospección).
        </p>
      </div>
