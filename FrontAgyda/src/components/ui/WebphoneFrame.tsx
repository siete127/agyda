  ...
  return (
    <>
      ...
      <div
        className={
          (inPip
            ? 'fixed z-40 flex flex-col overflow-hidden rounded-xl border border-gray-300 bg-white shadow-2xl ring-1 ring-black/5'
            : 'fixed z-40 overflow-hidden rounded-2xl border border-gray-200/60 bg-white shadow-xl') +
      ...
      >
        {inPip && (
          <div
            ...
            className="flex flex-shrink-0 cursor-grab items-center justify-between gap-1.5 bg-gray-900 pl-2 pr-1 text-white select-none active:cursor-grabbing"
      ...
      {loading && !loadError && (
        <div className="absolute inset-0 z-10 flex items-center justify-center bg-white" style={inPip ? { top: FLOAT_HEADER_HEIGHT } : undefined}>
          <span className="h-8 w-8 animate-spin rounded-full border-4 border-brand/20 border-t-brand" />
        </div>
      )}
      {loadError && (
        <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 bg-white px-6 text-center" style={inPip ? { top: FLOAT_HEADER_HEIGHT } : undefined}>
