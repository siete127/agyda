...
CREATE TABLE dbo.CRM_COTIZACION_ITEMS (
  ...
  const alters = [
    ...
    // Requerimientos/notas extendidas por renglón — usado por el flujo de
    // "Nueva cotización" del Portal de Cliente, donde el cliente detalla qué
    // necesita de cada producto/servicio elegido (distinto de COT_NOTAS, que
    // es una sola nota general de toda la cotización).
    `IF COL_LENGTH('dbo.CRM_COTIZACION_ITEMS','COTI_REQUERIMIENTOS') IS NULL ALTER TABLE dbo.CRM_COTIZACION_ITEMS ADD COTI_REQUERIMIENTOS NVARCHAR(MAX) NULL;`,
...
BEGIN
  CREATE TABLE dbo.CAP_INSCRIPCIONES (
    ...
    INSC_ASIGNADO_POR     INT             NULL,
    INSC_PARTICIPANTE_NUMERO NVARCHAR(40) NULL,
    INSC_PARTICIPANTE_NOMBRE NVARCHAR(150) NULL,
  ...
  CREATE UNIQUE INDEX UX_CAP_INSCRIPCIONES_CURSO_USUARIO ON dbo.CAP_INSCRIPCIONES(INSC_CURSO_ID, INSC_USUARIO_ID) WHERE INSC_USUARIO_ID IS NOT NULL;
END
`);
    // ── Migraciones incrementales (fuera del batch: los ALTER con drop de
    //    índice + ALTER COLUMN no conviven en un solo batch) ────────────────
    const alter = async (label, q) => {
      try { await pool.request().query(q); } catch (e) { console.warn(`⚠️ cap-schema ${label}:`, e.message); }
    };
    await alter('CUR_ACCESO', `IF COL_LENGTH('dbo.CAP_CURSOS','CUR_ACCESO') IS NULL ALTER TABLE dbo.CAP_CURSOS ADD CUR_ACCESO NVARCHAR(10) NOT NULL CONSTRAINT DF_CAP_CURSOS_ACCESO DEFAULT ('privado')`);
    await alter('CUR_SLUG_PUBLICO', `IF COL_LENGTH('dbo.CAP_CURSOS','CUR_SLUG_PUBLICO') IS NULL ALTER TABLE dbo.CAP_CURSOS ADD CUR_SLUG_PUBLICO NVARCHAR(60) NULL`);
    await alter('UX_CAP_CURSOS_SLUG', `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='UX_CAP_CURSOS_SLUG' AND object_id=OBJECT_ID('dbo.CAP_CURSOS')) CREATE UNIQUE INDEX UX_CAP_CURSOS_SLUG ON dbo.CAP_CURSOS(CUR_SLUG_PUBLICO) WHERE CUR_SLUG_PUBLICO IS NOT NULL`);
    await alter('INSC_ASIGNADO_POR', `IF COL_LENGTH('dbo.CAP_INSCRIPCIONES','INSC_ASIGNADO_POR') IS NULL ALTER TABLE dbo.CAP_INSCRIPCIONES ADD INSC_ASIGNADO_POR INT NULL`);
    await alter('INSC_PARTICIPANTE_NUMERO', `IF COL_LENGTH('dbo.CAP_INSCRIPCIONES','INSC_PARTICIPANTE_NUMERO') IS NULL ALTER TABLE dbo.CAP_INSCRIPCIONES ADD INSC_PARTICIPANTE_NUMERO NVARCHAR(40) NULL`);
    await alter('INSC_PARTICIPANTE_NOMBRE', `IF COL_LENGTH('dbo.CAP_INSCRIPCIONES','INSC_PARTICIPANTE_NOMBRE') IS NULL ALTER TABLE dbo.CAP_INSCRIPCIONES ADD INSC_PARTICIPANTE_NOMBRE NVARCHAR(150) NULL`);
    await alter('drop UQ inscripciones', `IF EXISTS (SELECT 1 FROM sys.key_constraints WHERE name='UQ_CAP_INSCRIPCIONES_CURSO_USUARIO' AND parent_object_id=OBJECT_ID('dbo.CAP_INSCRIPCIONES')) ALTER TABLE dbo.CAP_INSCRIPCIONES DROP CONSTRAINT UQ_CAP_INSCRIPCIONES_CURSO_USUARIO`);
    await alter('INSC_USUARIO_ID nullable', `IF EXISTS (SELECT 1 FROM sys.columns WHERE object_id=OBJECT_ID('dbo.CAP_INSCRIPCIONES') AND name='INSC_USUARIO_ID' AND is_nullable=0) ALTER TABLE dbo.CAP_INSCRIPCIONES ALTER COLUMN INSC_USUARIO_ID INT NULL`);
    await alter('UX inscripciones filtrado', `IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name='UX_CAP_INSCRIPCIONES_CURSO_USUARIO' AND object_id=OBJECT_ID('dbo.CAP_INSCRIPCIONES')) CREATE UNIQUE INDEX UX_CAP_INSCRIPCIONES_CURSO_USUARIO ON dbo.CAP_INSCRIPCIONES(INSC_CURSO_ID, INSC_USUARIO_ID) WHERE INSC_USUARIO_ID IS NOT NULL`);
