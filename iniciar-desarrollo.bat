...
REM
REM  Backend      -> http://localhost:8447
...
echo Iniciando Backend (BackAgyda) en puerto 8447...
start "Backend - BackAgyda (8447)" cmd /k "cd /d "%ROOT%BackAgyda" && npm run dev"
...
start "Pagina publica (8080)" cmd /k "cd /d "%ROOT%extra\Pagina de Intranet_1" && set DEV_PORT=8080 && set BACKEND_TARGET=http://127.0.0.1:8447 && npm run dev"

echo.
echo ============================================================
echo  Sistema iniciado. Abriendo en 3 ventanas separadas:
echo    Backend    -> http://localhost:8447
