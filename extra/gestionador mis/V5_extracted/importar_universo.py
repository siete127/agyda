...
COHORT_TABLE = "dbo.Vicidial_Lead_MonthlyCohort"
...
def normalize_entry_month(value: Optional[str]) -> Optional[str]:
    """Valida y normaliza un mes explícito en formato YYYY-MM."""
    text = clean_text(value)
    if not text:
        return None
    if not re.fullmatch(r"\d{4}-(0[1-9]|1[0-2])", text):
        raise ValueError("El mes de EntryDate debe tener el formato YYYY-MM.")
    return text


def entry_month_date(value: Optional[str]) -> Optional[str]:
    month = normalize_entry_month(value)
    return f"{month}-01 00:00:00" if month else None
...
def row_value(raw: Dict[str, object], *names: str) -> Optional[str]:
    """Obtiene una columna sin depender de mayúsculas, acentos o formato."""
    normalized = {_header_key(key): value for key, value in raw.items() if key is not None}
    for name in names:
        value = clean_text(normalized.get(_header_key(name)))
...
def csv_rows(
    path: Path,
    default_year: int,
    entry_month_override: Optional[str] = None,
) -> Iterator[Dict[str, object]]:
    campaign_hint, entry_hint, month_hint = infer_metadata(path.name, default_year)
    selected_month = normalize_entry_month(entry_month_override)
    selected_entry_date = entry_month_date(selected_month)
    ...
    with path.open("r", encoding=encoding, errors="replace", newline="") as f:
        ...
        header_lookup = {
            _header_key(header): header
            for header in (reader.fieldnames or [])
            if header is not None
        }
        for row_num, raw in enumerate(reader, start=2):
            def value(*names: str) -> Optional[str]:
                for name in names:
                    header = header_lookup.get(_header_key(name))
                    result = clean_text(raw.get(header)) if header is not None else None
            ...
            lead_id = parse_int(value("Lead", "lead_id"))
            if lead_id is None:
                lead_id = stable_negative_lead_id(path.name, row_num, phone_norm)
            call_date = parse_date(value("Fecha", "modify_date", "last_local_call_time"))
            disposition = value("Disposicion", "Status")
            disposition_name = value("Nombre Disposicion", "StatusDetalle")
            if disposition and disposition_name and disposition_name.casefold() != disposition.casefold():
                disposition_detail = f"{disposition} - {disposition_name}"
            else:
                disposition_detail = disposition_name or disposition
            list_id = value("Lista", "list_id")
            campaign = normalize_campaign(
                value("Campana", "Campaña", "campaign_id") or campaign_hint
            )
            called_count = parse_int(value("Conteo Llamadas", "called_count"))
            if called_count is None and call_date:
                called_count = 1
            management_month = call_date[:7] if call_date else selected_month or month_hint
            entry_date = selected_entry_date or call_date or entry_hint
            comments_parts = []
            ...
            values = {
                ...
                "StatusDetalle": disposition_detail,
                "StatusFuente": "CALL_REPORT_ARCHIVO",
                "UserName": value("Agente", "user"),
                "VendorLeadCode": value("CENTRAL DE CREDITO", "vendor_lead_code"),
                "SourceID": value("CUENTA", "Cuenta", "source_id"),
                "ListID": list_id,
                "PhoneCode": "1" if phone_norm else None,
                "PhoneNumber": phone,
                "FirstName": value("NOMBRE", "Nombre", "first_name"),
                "LastName": value("Apellidos", "last_name"),
                "State": value("Estado", "state"),
                "AltPhone": value("Telefono 2", "CELULAR", "alt_phone"),
                "Email": value("CORREO", "Email", "email"),
                ...
                "UltimoMesGestion": management_month,
            }
            values["SourceRowHash"] = row_hash(
                [path.name, row_num]
                + [
                    values[c]
                    for c in STAGE_COLUMNS
                    if c not in ("SourceFile", "SourceRow", "SourceRowHash")
                ]
            )
            yield values
...
def xlsx_rows(
    ...
    selected_month = normalize_entry_month(entry_month_override)
    if selected_month:
        entry_hint = entry_month_date(selected_month)
        month_hint = selected_month
...
END;
IF COL_LENGTH('{CONTROL_TABLE}', 'EntryMonth') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD EntryMonth CHAR(7) NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'TelefonosUnicos') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD TelefonosUnicos BIGINT NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'TelefonosNuevos') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD TelefonosNuevos BIGINT NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'TelefonosExistentes') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD TelefonosExistentes BIGINT NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'ExistentesMesAnterior') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD ExistentesMesAnterior BIGINT NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'MarcadosAntes') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD MarcadosAntes BIGINT NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'ExistentesMismoMes') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD ExistentesMismoMes BIGINT NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'FilasInvalidas') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD FilasInvalidas BIGINT NULL;
IF COL_LENGTH('{CONTROL_TABLE}', 'InteraccionesDuplicadas') IS NULL
    ALTER TABLE {CONTROL_TABLE} ADD InteraccionesDuplicadas BIGINT NULL;
IF OBJECT_ID('{COHORT_TABLE}', 'U') IS NULL
BEGIN
    CREATE TABLE {COHORT_TABLE}(
        CohortID BIGINT IDENTITY(1,1) NOT NULL PRIMARY KEY,
        EntryMonth CHAR(7) NOT NULL,
        ListID NVARCHAR(100) NOT NULL CONSTRAINT DF_VicidialCohort_List DEFAULT '',
        PhoneNormalized VARCHAR(20) NOT NULL,
        LeadID BIGINT NULL,
        SourceFile NVARCHAR(260) NULL,
        FirstControlID BIGINT NULL,
        LastControlID BIGINT NULL,
        WasExistingBefore BIT NOT NULL CONSTRAINT DF_VicidialCohort_Existing DEFAULT 0,
        ExistedPriorMonth BIT NOT NULL CONSTRAINT DF_VicidialCohort_Prior DEFAULT 0,
        WasDialedBefore BIT NOT NULL CONSTRAINT DF_VicidialCohort_Dialed DEFAULT 0,
        FirstSeenAt DATETIME2(0) NOT NULL CONSTRAINT DF_VicidialCohort_FirstSeen DEFAULT SYSDATETIME(),
        LastSeenAt DATETIME2(0) NOT NULL CONSTRAINT DF_VicidialCohort_LastSeen DEFAULT SYSDATETIME()
    );
    CREATE UNIQUE INDEX UX_VicidialCohort_MonthListPhone
        ON {COHORT_TABLE}(EntryMonth,ListID,PhoneNormalized);
    CREATE INDEX IX_VicidialCohort_PhoneMonth
        ON {COHORT_TABLE}(PhoneNormalized,EntryMonth)
...
def analyze_source_history(
    ...
    """
    Clasifica los teléfonos del archivo antes del upsert.

    La clasificación es informativa: ningún teléfono se elimina, bloquea ni se
    agrega a ControlLotesVicidial por haber aparecido en meses anteriores.
    """
    target = validate_table_name(target)
    selected_month = normalize_entry_month(entry_month)
    if not selected_month:
        raise ValueError("Selecciona el mes de EntryDate para analizar el historial.")

    unique_phones = prepare_source_snapshot(conn, source_files)
    cursor = conn.cursor()
    source_row = cursor.execute(f"""
SELECT
    COUNT_BIG(*) AS Filas,
    SUM(CASE WHEN s.PhoneNormalized IS NULL THEN CONVERT(bigint,1) ELSE 0 END) AS Invalidas,
    COUNT_BIG(DISTINCT CASE WHEN s.PhoneNormalized IS NOT NULL THEN s.PhoneNormalized END) AS Telefonos
FROM {STAGE_TABLE} s
WHERE EXISTS (
    SELECT 1 FROM #SelectedSourceFiles sf WHERE sf.SourceFile=s.SourceFile
);
""").fetchone()
    source_rows = int(source_row[0] or 0)
    invalid_rows = int(source_row[1] or 0)
    valid_rows = max(0, source_rows - invalid_rows)

    # La tabla temporal se crea sin parámetros para conservarla en el alcance
    # de la conexión ODBC y reutilizarla al registrar la membresía mensual.
    cursor.execute("""
IF OBJECT_ID('tempdb..#SourceClassification') IS NOT NULL
    DROP TABLE #SourceClassification;
SELECT TOP (0)
        s.PhoneNormalized,
        s.ListID,
        s.LeadID,
        s.SourceFile,
        CONVERT(int,0) AS EsNuevo,
        CONVERT(int,0) AS YaExiste,
        CONVERT(int,0) AS ExistiaMesAnterior,
        CONVERT(int,0) AS MarcadoAntes
INTO #SourceClassification
FROM #SrcLatest s;
CREATE UNIQUE CLUSTERED INDEX IX_SourceClassification_Phone
    ON #SourceClassification(PhoneNormalized);
""")

    row = cursor.execute(f"""
SET NOCOUNT ON;
DECLARE @MonthStart date = ?;
INSERT INTO #SourceClassification
SELECT
        s.PhoneNormalized,
        s.ListID,
        s.LeadID,
        s.SourceFile,
        CASE WHEN t.PhoneNormalized IS NULL THEN 1 ELSE 0 END AS EsNuevo,
        CASE WHEN t.PhoneNormalized IS NOT NULL THEN 1 ELSE 0 END AS YaExiste,
        CASE WHEN
            COALESCE(t.FirstSeenDate,t.EntryDate,t.ModifyDate,t.LastLocalCallTime) < @MonthStart
            OR EXISTS (
                SELECT 1
                FROM {STAGE_TABLE} h
                WHERE h.PhoneNormalized=s.PhoneNormalized
                  AND NOT EXISTS (
                      SELECT 1 FROM #SelectedSourceFiles sf
                      WHERE sf.SourceFile=h.SourceFile
                  )
                  AND COALESCE(h.EntryDate,h.ModifyDate,h.LastLocalCallTime) < @MonthStart
            )
            THEN 1 ELSE 0
        END AS ExistiaMesAnterior,
        CASE WHEN
            (
                ISNULL(t.CalledCount,0) > 0
                AND COALESCE(t.LastLocalCallTime,t.LastInteractionDate,t.ModifyDate,t.EntryDate) < @MonthStart
            )
            OR EXISTS (
                SELECT 1
                FROM {STAGE_TABLE} h
                WHERE h.PhoneNormalized=s.PhoneNormalized
                  ...
                  AND COALESCE(h.LastLocalCallTime,h.ModifyDate,h.EntryDate) < @MonthStart
                  AND (
                      ISNULL(h.CalledCount,0) > 0
                      OR h.LastLocalCallTime IS NOT NULL
                      OR UPPER(COALESCE(h.Status,'')) NOT IN ('','NEW','NUEVO')
                  )
            )
            THEN 1 ELSE 0
        END AS MarcadoAntes
FROM #SrcLatest s
LEFT JOIN {target} t
  ON t.PhoneNormalized=s.PhoneNormalized;

SELECT
    COUNT_BIG(*) AS TelefonosUnicos,
    SUM(CONVERT(bigint,EsNuevo)) AS TelefonosNuevos,
    SUM(CONVERT(bigint,YaExiste)) AS TelefonosExistentes,
    SUM(CONVERT(bigint,ExistiaMesAnterior)) AS ExistentesMesAnterior,
    SUM(CONVERT(bigint,MarcadoAntes)) AS MarcadosAntes,
    SUM(CASE WHEN YaExiste=1 AND ExistiaMesAnterior=0 THEN CONVERT(bigint,1) ELSE 0 END)
        AS ExistentesMismoMes
FROM #SourceClassification;
""", f"{selected_month}-01").fetchone()

    return {
        "source_rows": source_rows,
        "unique_phones": int(row[0] or unique_phones or 0),
        "new_phones": int(row[1] or 0),
        "existing_phones": int(row[2] or 0),
        "existing_previous_month": int(row[3] or 0),
        "previously_dialed": int(row[4] or 0),
        "existing_same_month": int(row[5] or 0),
        "invalid_rows": invalid_rows,
        "duplicate_interactions": max(0, valid_rows - int(row[0] or 0)),
    }


def upsert_monthly_cohort(
    conn: pyodbc.Connection,
    control_id: int,
    entry_month: str,
    commit: bool = True,
) -> Tuple[int, int]:
    """Registra la pertenencia mensual sin duplicar el teléfono maestro."""
    selected_month = normalize_entry_month(entry_month)
    if not selected_month:
        raise ValueError("Selecciona el mes de EntryDate para registrar la cohorte.")
    cursor = conn.cursor()
    if not cursor.execute(
        "SELECT CASE WHEN OBJECT_ID('tempdb..#SourceClassification') IS NULL THEN 0 ELSE 1 END"
    ).fetchval():
        raise RuntimeError("No existe el análisis mensual previo a la importación.")
    cursor.execute(f"""
IF OBJECT_ID('tempdb..#MonthlyMembershipSource') IS NOT NULL
    DROP TABLE #MonthlyMembershipSource;
;WITH Ranked AS (
    SELECT
        s.PhoneNormalized,
        COALESCE(CONVERT(nvarchar(100),s.ListID),'') AS ListID,
        s.LeadID,
        s.SourceFile,
        c.YaExiste,
        c.ExistiaMesAnterior,
        c.MarcadoAntes,
        ROW_NUMBER() OVER (
            PARTITION BY s.PhoneNormalized,COALESCE(CONVERT(nvarchar(100),s.ListID),'')
            ORDER BY
                COALESCE(s.LastLocalCallTime,s.ModifyDate,s.EntryDate,s.FechaStage) DESC,
                s.StageID DESC
        ) AS rn
    FROM {STAGE_TABLE} s
    INNER JOIN #SourceClassification c
      ON c.PhoneNormalized=s.PhoneNormalized
    WHERE s.PhoneNormalized IS NOT NULL
      AND EXISTS (
          SELECT 1 FROM #SelectedSourceFiles sf
          WHERE sf.SourceFile=s.SourceFile
      )
)
SELECT
    PhoneNormalized,ListID,LeadID,SourceFile,
    YaExiste,ExistiaMesAnterior,MarcadoAntes
INTO #MonthlyMembershipSource
FROM Ranked
WHERE rn=1;
CREATE UNIQUE CLUSTERED INDEX IX_MonthlyMembershipSource
    ON #MonthlyMembershipSource(ListID,PhoneNormalized);
""")
    source_count = int(
        cursor.execute("SELECT COUNT_BIG(*) FROM #MonthlyMembershipSource").fetchval() or 0
    )

    existing = int(cursor.execute(f"""
SELECT COUNT_BIG(*)
FROM #MonthlyMembershipSource s
INNER JOIN {COHORT_TABLE} c
  ON c.EntryMonth=?
 AND c.ListID=s.ListID
 AND c.PhoneNormalized=s.PhoneNormalized;
""", selected_month).fetchval() or 0)

    cursor.execute(f"""
UPDATE c
SET c.LastControlID=?,
    c.LeadID=COALESCE(s.LeadID,c.LeadID),
    c.SourceFile=COALESCE(s.SourceFile,c.SourceFile),
    c.WasExistingBefore=CASE WHEN c.WasExistingBefore=1 OR s.YaExiste=1 THEN 1 ELSE 0 END,
    c.ExistedPriorMonth=CASE WHEN c.ExistedPriorMonth=1 OR s.ExistiaMesAnterior=1 THEN 1 ELSE 0 END,
    c.WasDialedBefore=CASE WHEN c.WasDialedBefore=1 OR s.MarcadoAntes=1 THEN 1 ELSE 0 END,
    c.LastSeenAt=SYSDATETIME()
FROM {COHORT_TABLE} c
INNER JOIN #MonthlyMembershipSource s
  ON c.EntryMonth=?
 AND c.ListID=s.ListID
 AND c.PhoneNormalized=s.PhoneNormalized;
""", int(control_id), selected_month)

    cursor.execute(f"""
INSERT INTO {COHORT_TABLE}(
    EntryMonth,ListID,PhoneNormalized,LeadID,SourceFile,
    FirstControlID,LastControlID,WasExistingBefore,ExistedPriorMonth,WasDialedBefore
)
SELECT
    ?,s.ListID,s.PhoneNormalized,s.LeadID,s.SourceFile,
    ?,?,CONVERT(bit,s.YaExiste),CONVERT(bit,s.ExistiaMesAnterior),CONVERT(bit,s.MarcadoAntes)
FROM #MonthlyMembershipSource s
WHERE NOT EXISTS (
    SELECT 1
    FROM {COHORT_TABLE} c WITH (UPDLOCK,HOLDLOCK)
    WHERE c.EntryMonth=?
      AND c.ListID=s.ListID
      AND c.PhoneNormalized=s.PhoneNormalized
);
""", selected_month, int(control_id), int(control_id), selected_month)
    inserted = max(0, source_count - existing)
    if commit:
        conn.commit()
    return inserted, existing


def start_import_control(
    conn: pyodbc.Connection,
    source_file: str,
    entry_month: Optional[str],
) -> int:
    cursor = conn.cursor()
    row = cursor.execute(f"""
INSERT INTO {CONTROL_TABLE}(SourceFile,EntryMonth,Estado)
OUTPUT INSERTED.ControlID
VALUES (?,?, 'INICIADO');
""", str(source_file)[:260], normalize_entry_month(entry_month)).fetchone()
    conn.commit()
    return int(row[0])


def finish_import_control(
    conn: pyodbc.Connection,
    control_id: int,
    status: str,
    read: int,
...
SET FechaFin=SYSDATETIME(),
    ...
    TelefonosNuevos=?,
    TelefonosExistentes=?,
    ExistentesMesAnterior=?,
    MarcadosAntes=?,
    ExistentesMismoMes=?,
    FilasInvalidas=?,
    InteraccionesDuplicadas=?,
    Estado=?,
    Mensaje=?
WHERE ControlID=?;
""",
        ...
        int(inserted or 0),
        int(updated or 0),
        int(stats.get("unique_phones", 0)),
        int(stats.get("new_phones", 0)),
        int(stats.get("existing_phones", 0)),
        int(stats.get("existing_previous_month", 0)),
        int(stats.get("previously_dialed", 0)),
        int(stats.get("existing_same_month", 0)),
        int(stats.get("invalid_rows", 0)),
        int(stats.get("duplicate_interactions", 0)),
        str(status)[:20],
        str(message or "")[:2000] or None,
        int(control_id),
    )
    conn.commit()
...
SET
    ...
    T.FirstSeenDate = CASE
        WHEN COALESCE(T.FirstSeenDate,T.EntryDate) IS NULL
          OR S.FirstSeenMin < COALESCE(T.FirstSeenDate,T.EntryDate)
        THEN S.FirstSeenMin
        ELSE COALESCE(T.FirstSeenDate,T.EntryDate)
    END,
...
SELECT
    S.LeadID, COALESCE(S.EntryDate,S.InteractionMax), COALESCE(S.ModifyDate, S.InteractionMax), COALESCE(S.Status, 'NEW'),
...
def process_file(
    ...
    if suffix in (".csv", ".txt"):
        records = csv_rows(path, year, entry_month_override)
    elif suffix in (".xlsx", ".xlsm"):
        records = xlsx_rows(path, year, entry_month_override)
    else:
        records = legacy_excel_rows(path, year, entry_month_override)
