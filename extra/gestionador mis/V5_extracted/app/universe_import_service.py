...
from app.universe_priority_service import invalidate_universe_priority_snapshot
...
def reserve_job(
    ...
    job = {
        ...
        "filename": filename,
        "year": year,
        "entry_month": importer.normalize_entry_month(entry_month),
        "size": size,
        "status": "SUBIENDO",
        ...
        "new_phones": 0,
        "existing_phones": 0,
        "existing_previous_month": 0,
        "previously_dialed": 0,
        "existing_same_month": 0,
        "invalid_rows": 0,
...
def _run_job(job_id: str) -> None:
    ...
    read = staged = inserted = updated = 0
    analysis: Dict[str, int] = {}
    cohort_inserted = cohort_existing = 0
    try:
        with _jobs_lock:
            job = _jobs[job_id]
            file_path = Path(str(job["file_path"]))
            year = int(job["year"])
            entry_month = importer.normalize_entry_month(job.get("entry_month"))
        ...
        try:
            ...
            control_id = importer.start_import_control(
                connection,
                file_path.name,
                entry_month,
            )
            ...
            read, staged = importer.process_input(
                ...
                entry_month_override=entry_month,
            )
            with _jobs_lock:
                _jobs[job_id]["read"] = read
                _jobs[job_id]["staged"] = staged
                _jobs[job_id]["phase"] = "Consolidando y actualizando universo"
                _jobs[job_id]["progress_percent"] = 78
                _jobs[job_id]["eta_seconds"] = None
            if _cancel_requested(job_id):
                raise importer.ImportCancelled("Importación detenida por el usuario.")

            with _jobs_lock:
                _jobs[job_id]["phase"] = "Comparando historial de meses anteriores"
                _jobs[job_id]["progress_percent"] = 84
                _jobs[job_id]["can_cancel"] = False
            analysis = importer.analyze_source_history(
                connection,
                "dbo.Vicidial_Leads_Completo",
                source_files,
                entry_month or f"{year:04d}-01",
            )
            with _jobs_lock:
                _jobs[job_id].update(analysis)
                _jobs[job_id]["phase"] = "Actualizando universo sin bloquear coincidencias"
                _jobs[job_id]["progress_percent"] = 91

            inserted, updated = importer.update_insert_target(
                connection, "dbo.Vicidial_Leads_Completo", source_files
            )
            cohort_inserted, cohort_existing = importer.upsert_monthly_cohort(
                connection,
                control_id,
                entry_month or f"{year:04d}-01",
            )
            with _jobs_lock:
                _jobs[job_id]["cohort_inserted"] = cohort_inserted
                _jobs[job_id]["cohort_existing"] = cohort_existing
            if control_id is not None:
                importer.finish_import_control(
                    connection,
                    control_id,
                    "COMPLETADO",
                    read,
                    ...
                    analysis,
                    (
                        "Importación mensual terminada sin bloquear teléfonos históricos. "
                        f"Membresías nuevas: {cohort_inserted}; existentes: {cohort_existing}."
                    ),
                )
            ...
            if control_id is not None:
                try:
                    ...
                    importer.finish_import_control(
                        connection,
                        control_id,
                        "CANCELADO" if isinstance(exc, importer.ImportCancelled) else "ERROR",
                        read,
                        staged,
                        inserted,
                        updated,
        ...
        try:
            ...
            logging.getLogger(__name__).exception(
                "La carga terminó, pero no se pudo invalidar el KPI del universo"
            )
