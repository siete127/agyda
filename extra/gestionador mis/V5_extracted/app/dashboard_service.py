...
from app.sql_loader import connection_scope, q, split_table_name
...
def _azul_leads_table() -> str:
    return _qualified_table(config.AZUL_SNAPSHOT_TABLE, config.SQL_DATABASE)


def _azul_lists_table() -> str:
    return _qualified_table(config.AZUL_LISTS_TABLE, config.SQL_DATABASE)


def _clean_list_text(expression: str) -> str:
    """Normaliza textos vacíos o literales NULL provenientes de Vicidial."""
    return (
        "NULLIF(NULLIF(LTRIM(RTRIM(CONVERT(nvarchar(255),"
        + expression
        + "))),''),'NULL')"
    )


def _resolved_list_name_sql(
    source_name_expression: str,
    list_id_expression: str,
    catalog_alias: str,
) -> str:
    """Prioriza siempre el nombre oficial del catálogo Vicidial."""
    return (
        "COALESCE("
        + _clean_list_text(catalog_alias + ".[list_name]")
        + ","
        + _clean_list_text(source_name_expression)
        + ","
        + _clean_list_text(list_id_expression)
        + ")"
    )
...
def _prepare_fusion(cursor, selected) -> Tuple[str, str, str]:
    ...
    azul_leads = _azul_leads_table()
    azul_lists = _azul_lists_table()
    ...
    azul_phone = _phone_expr(
        "COALESCE(CONVERT(varchar(80),az.[phone_number]),'')"
    )
    azul_list_name = _resolved_list_name_sql(
        "NULL",
        "az.[list_id]",
        "vl",
    )
...
FROM (
    SELECT
        TRY_CONVERT(bigint,cr.[ImportID]) AS ImportID,
        CONVERT(nvarchar(40),cr.[call_date]) AS call_date,
        CONVERT(nvarchar(80),cr.[status]) AS status,
        CONVERT(nvarchar(80),cr.[user]) AS [user],
        CONVERT(nvarchar(220),cr.[full_name]) AS full_name,
        CONVERT(nvarchar(80),cr.[campaign_id]) AS campaign_id,
        CONVERT(nvarchar(80),cr.[list_id]) AS list_id,
        CONVERT(nvarchar(255),cr.[list_name]) AS list_name,
        CONVERT(nvarchar(500),cr.[list_description]) AS list_description,
        CONVERT(nvarchar(80),cr.[lead_id]) AS lead_id,
        CONVERT(nvarchar(80),cr.[phone_number]) AS phone_number,
        CONVERT(nvarchar(80),cr.[phone_number_dialed]) AS phone_number_dialed,
        CONVERT(nvarchar(200),cr.[status_name]) AS status_name,
        CONVERT(nvarchar(160),cr.[uniqueid]) AS uniqueid,
        {call_dt} AS CallDateTime,
        {call_phone} AS TelefonoNormalizado,
        CONVERT(tinyint,1) AS SourcePriority
    FROM {calls} cr WITH (NOLOCK)
    ...
    SELECT
        CONVERT(bigint,-6003500000000000000)
            - ABS(COALESCE(TRY_CONVERT(bigint,az.[lead_id]),CONVERT(bigint,CHECKSUM(az.[phone_number])))),
        CONVERT(nvarchar(40),{azul_dt},120),
        CONVERT(nvarchar(80),az.[status]),
        CONVERT(nvarchar(80),az.[user]),
        CONVERT(nvarchar(220),LTRIM(RTRIM(CONCAT(COALESCE(az.[first_name],''),' ',COALESCE(az.[last_name],''))))),
        CONVERT(nvarchar(80),COALESCE(NULLIF(CONVERT(nvarchar(80),vl.[campaign_id]),''),?)),
        CONVERT(nvarchar(80),az.[list_id]),
        CONVERT(nvarchar(255),{azul_list_name}),
        CONVERT(nvarchar(500),vl.[list_description]),
        CONVERT(nvarchar(80),az.[lead_id]),
        CONVERT(nvarchar(80),az.[phone_number]),
        CONVERT(nvarchar(80),az.[phone_number]),
        CONVERT(nvarchar(200),az.[status]),
        CONVERT(nvarchar(160),CONCAT('AZUL-',az.[lead_id])),
        {azul_dt},
        {azul_phone},
        CONVERT(tinyint,2)
    FROM {azul_leads} az WITH (NOLOCK)
    LEFT JOIN {azul_lists} vl WITH (NOLOCK)
      ON TRY_CONVERT(decimal(20,0),vl.[list_id])
       = TRY_CONVERT(decimal(20,0),az.[list_id])
    WHERE {azul_dt} >= DATEADD(DAY, -{int(config.PHONE_MATCH_DAYS)}, CAST(? AS datetime2(0)))
      AND {azul_dt} < DATEADD(DAY, {int(config.PHONE_MATCH_DAYS) + 1}, CAST(? AS datetime2(0)))
      AND (ISNULL(TRY_CONVERT(int,az.[called_count]),0)>0 OR az.[last_local_call_time] IS NOT NULL)
      AND (vl.[list_id] IS NULL OR CONVERT(nvarchar(80),vl.[campaign_id])=?)
) source;
...
INCLUDE (ImportID, status, [user], full_name, campaign_id, list_id, list_name,
         list_description, lead_id, phone_number, phone_number_dialed, status_name,
         uniqueid, call_date, SourcePriority);
